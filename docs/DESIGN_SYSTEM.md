...
- Global `:focus-visible` ring (`border-border-focus`, `@layer base` in
  ...
  need to hand-roll focus styles. Form inputs that use `focus:outline-none` +
  a `focus:border-hud-blue` color change instead of the ring are an
  intentional, WCAG-compliant alternate focus indicator, not a gap.
- Reduced motion is handled by a single universal-selector override in
  `src/index.css` (`@layer base`, `@media (prefers-reduced-motion: reduce)`)
  that zeroes `animation-duration`/`transition-duration` on `*` with
  `!important` -- this catches every `animate-*` Tailwind class and inline
  `style={{ animationDelay: ... }}` override by construction, so no
  per-component `motion-reduce:` variant is needed. The one exception is
  `gamification/Confetti.jsx`, which drives its burst via Canvas +
  `requestAnimationFrame` (not a CSS animation, so the override can't reach
  it) -- it checks `matchMedia('(prefers-reduced-motion: reduce)')` directly
  and skips the burst.
- Contrast matrix (`node scripts/contrastAudit.mjs`, run 2026-07-03): every
  semantic foreground/background pair clears its WCAG AA floor (4.5:1 body
  text, 3:1 non-text/large-bold). The one sub-4.5 result --
  `hud.pink` on `bg.raised` = 3.86:1 -- is the documented policy exception
  above (restricted to large/bold/chrome; use `accent.pink-text` for body
  copy on raised surfaces). Re-run the script after any hex change; it exits
  non-zero on any *undocumented* regression.
- axe-core sweep: `docs/ux-overhaul/A11Y.md` (generated by
  `scripts/uxAudit.a11y.mjs`), 4 views x 3 widths, exit bar 0 serious/critical
  violations.
