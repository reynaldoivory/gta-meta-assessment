...
import { Target, TrendingUp, AlertTriangle, CheckCircle2, DollarSign, Info, Zap, AlertCircle, Shield } from 'lucide-react';
...
/**
 * Determines the top priority action for the player
 * @param {Object} data - Form data
 * @param {Object} results - Calculation results
 * @returns {Object} - Priority action object
 */
const getTopPriorityAction = (data, results) => {
  // Check for critically low stats
  const stats = [
    { name: 'Strength', value: validateStat(data.strengthStat) },
    { name: 'Stealth', value: validateStat(data.stealthStat) },
    { name: 'Shooting', value: validateStat(data.shootingStat) },
    { name: 'Driving', value: validateStat(data.drivingStat) },
    { name: 'Flying', value: validateStat(data.flyingStat) }
  ];

  const lowestStat = stats.reduce((min, stat) => stat.value < min.value ? stat : min, stats[0]);

  if (lowestStat.value < 60) {
    return {
      level: 'CRITICAL',
      icon: '⚠️',
      title: `Train ${lowestStat.name} Stat (Currently ${lowestStat.value}/100)`,
      reason: `Low ${lowestStat.name.toLowerCase()} severely impacts your efficiency in missions and combat`,
      steps: lowestStat.name === 'Strength' 
        ? 'Complete Gerald missions, fight NPCs, or play Golf to increase strength'
        : lowestStat.name === 'Stealth'
        ? 'Sneak around in missions, crouch-walk consistently to level stealth'
        : lowestStat.name === 'Shooting'
        ? 'Visit Ammu-Nation shooting range or complete Survival missions'
        : lowestStat.name === 'Driving'
        ? 'Complete races or drive consistently (avoid crashes to gain XP faster)'
        : 'Complete flight school or fly helicopters/planes regularly'
    };
  }

  // No Kosatka - highest priority after stats
  if (!data.hasKosatka) {
    return {
      level: 'NEXT',
      icon: '🎯',
      title: 'Save $2.2M for Kosatka Submarine',
      reason: 'Kosatka unlocks Cayo Perico Heist - the best solo income method ($1M+/hr potential)',
      steps: 'Grind VIP Work (Sightseer, Headhunter), complete Contact Missions, or run Agency contracts if available'
    };
  }

  // Has Kosatka but no Sparrow
  if (data.hasKosatka && !data.hasSparrow) {
    return {
      level: 'NEXT',
      icon: '🚁',
      title: 'Buy Sparrow Helicopter ($1.8M)',
      reason: 'Sparrow cuts Cayo prep time by 10-15 minutes, dramatically improving hourly income',
      steps: 'Access Kosatka Moon Pool Vehicle Workshop and purchase Sparrow with missiles'
    };
  }

  // Has Kosatka but low experience
  const cayoCount = validateNumericInput(data.cayoCompletions, 0, 10000);
  if (data.hasKosatka && cayoCount < 10) {
    return {
      level: 'NEXT',
      icon: '🏝️',
      title: `Complete More Cayo Heists (${cayoCount}/10+)`,
      reason: 'Practice reduces your run time significantly - experienced players finish in <50 minutes',
      steps: 'Focus on drainage tunnel entry, grab primary + 2 secondary targets, use guard routes efficiently'
    };
  }
  ...
  if (!data.hasAgency) {
    return {
      level: 'NEXT',
      icon: '💼',
      title: 'Buy Agency ($2.1M base)',
      reason: 'Unlocks $1M Dre Contract, $85K Payphone Hits, and passive safe income (~$25K/hr)',
      steps: 'Purchase Agency property, complete Dre Contract storyline, unlock Payphone Hits'
    };
  }

  // Has action plan items
  if (results && results.actionPlan && results.actionPlan.length > 0) {
    const firstAction = results.actionPlan[0];
    return {
      level: 'NEXT',
      icon: '📋',
      title: firstAction.split(':')[0],
      reason: 'Address this bottleneck to increase your income efficiency',
      steps: firstAction.includes(':') ? firstAction.split(':')[1].trim() : firstAction
    };
  }
  ...
  return {
    level: 'OPTIMIZED',
    icon: '✅',
    title: 'Setup Complete - Run Your Grind Loop',
    reason: 'Your businesses and assets are well-optimized for income generation',
    steps: results && results.grindLoop && results.grindLoop[0] 
      ? results.grindLoop[0].replace(/^\d+\.\s*/, '')
      : 'Follow the optimized grind loop below to maximize income'
  };
};
...
export default function GTAMetaAssessment() {
  ...
  const calculateAssessment = useCallback(() => {
    try {
      ...
      // Calculate heist readiness
      const coreStats = [
        validateStat(formData.strengthStat),
        validateStat(formData.shootingStat),
        validateStat(formData.drivingStat),
        validateStat(formData.flyingStat)
      ];

      const heistReady = {
        statFloor: coreStats.every(stat => stat >= 60),
        cayoUnlocked: formData.hasKosatka,
        cayoExperienced: validateNumericInput(formData.cayoCompletions, 0, 10000) >= 10,
        mobility: formData.hasSparrow || formData.hasOppressor,
        infrastructure: formData.hasAgency && (formData.hasAcidLab || formData.hasNightclub || formData.hasBunker)
      };

      const heistReadyCriteria = Object.values(heistReady);
      const heistReadyScore = (heistReadyCriteria.filter(Boolean).length / heistReadyCriteria.length) * 100;
  ...
  // Calculate top priority action when results exist
  const topPriority = useMemo(() => {
    if (!results) return null;
    return getTopPriorityAction(formData, results);
  }, [formData, results]);
            ...
            <div className="mt-8 space-y-6" role="region" aria-label="Assessment Results">
              ...
              {topPriority && (
                <div className={`rounded-lg p-6 border-2 ${
                  topPriority.level === 'CRITICAL' 
                    ? 'bg-gradient-to-br from-red-900/40 to-red-800/20 border-red-500/50' 
                    : topPriority.level === 'NEXT'
                    ? 'bg-gradient-to-br from-blue-900/40 to-orange-800/20 border-blue-500/50'
                    : 'bg-gradient-to-br from-green-900/40 to-emerald-800/20 border-green-500/50'
                }`}>
                  <div className="flex items-start gap-4">
                    <div className="text-5xl" aria-hidden="true">{topPriority.icon}</div>
                    <div className="flex-1">
                      ...
                      <h3 className="text-white text-lg font-semibold mb-2">{topPriority.title}</h3>
                      <p className={`mb-3 ${
                        topPriority.level === 'CRITICAL'
                          ? 'text-red-100'
                          : topPriority.level === 'NEXT'
                        ...
                        <strong>Why:</strong> {topPriority.reason}
                      </p>
                      <p className={`${
                        topPriority.level === 'CRITICAL'
                          ? 'text-red-50'
                        ...
                        <strong>Action:</strong> {topPriority.steps}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-slate-700/30 rounded-lg p-6 border border-cyan-500/30">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className="text-xl text-cyan-400 font-bold">Heist Readiness</h3>
                </div>
                <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
                  <div className="flex-shrink-0">
                    <div className={`text-5xl font-bold ${
                      results.heistReadyScore === 100 ? 'text-green-400' :
                      results.heistReadyScore >= 60 ? 'text-yellow-400' : 'text-red-400'
                    }`}>
                      {Math.round(results.heistReadyScore)}%
                    </div>
                    <p className="text-gray-400 text-sm mt-1">Ready Score</p>
                  </div>
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="flex items-center gap-2">
                      {results.heistReady.statFloor ? (
                      ...
                      <span className={results.heistReady.statFloor ? 'text-green-300' : 'text-red-300'}>
                        Core Stats ≥60
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      {results.heistReady.cayoUnlocked ? (
                        <CheckCircle2 className="w-5 h-5 text-green-400 flex-shrink-0" aria-hidden="true" />
                      ) : (
                        <AlertCircle className="w-5 h-5 text-red-400 flex-shrink-0" aria-hidden="true" />
                      )}
                      <span className={results.heistReady.cayoUnlocked ? 'text-green-300' : 'text-red-300'}>
                        Kosatka Owned
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      {results.heistReady.cayoExperienced ? (
                        <CheckCircle2 className="w-5 h-5 text-green-400 flex-shrink-0" aria-hidden="true" />
                      ) : (
                        <AlertCircle className="w-5 h-5 text-red-400 flex-shrink-0" aria-hidden="true" />
                      )}
                      <span className={results.heistReady.cayoExperienced ? 'text-green-300' : 'text-red-300'}>
                        10+ Cayo Runs
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      {results.heistReady.mobility ? (
                        <CheckCircle2 className="w-5 h-5 text-green-400 flex-shrink-0" aria-hidden="true" />
                      ) : (
                        <AlertCircle className="w-5 h-5 text-red-400 flex-shrink-0" aria-hidden="true" />
                      )}
                      <span className={results.heistReady.mobility ? 'text-green-300' : 'text-red-300'}>
                        Fast Vehicle (Sparrow/Mk2)
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      {results.heistReady.infrastructure ? (
                        <CheckCircle2 className="w-5 h-5 text-green-400 flex-shrink-0" aria-hidden="true" />
                      ) : (
                        <AlertCircle className="w-5 h-5 text-red-400 flex-shrink-0" aria-hidden="true" />
                      )}
                      <span className={results.heistReady.infrastructure ? 'text-green-300' : 'text-red-300'}>
                        Support Infrastructure
                      </span>
                    </div>
                  </div>
                </div>
                {results.heistReadyScore < 100 && (
                  <p className="mt-4 text-cyan-200 text-sm bg-cyan-900/20 rounded p-3">
                    💡 Fix the top priority action above and re-calculate to improve your readiness score
                  </p>
                )}
              </div>
