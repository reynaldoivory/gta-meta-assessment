...
import { getLevelInfo, MILESTONE_LEVELS, getMilestoneLabel } from '../../utils/gamificationEngine';
...
const EmpireProgressPanel = ({ gamification }) => {
  ...
  const nextMilestone = useMemo(
    () => MILESTONE_LEVELS.find((m) => m > levelInfo.level) || null,
    [levelInfo.level]
  );
  ...
  const dailyQuests = gamification.quests?.daily?.items || [];
  const weeklyQuests = gamification.quests?.weekly?.items || [];
  const specialOps = gamification.quests?.specialOps?.items || [];
  const lastAward = gamification.lastAward;
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        <div className="rounded-2xl border border-slate-800/80 bg-slate-950/50 p-4">
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">
            <Target className="h-4 w-4 text-cyan-300" /> Daily Ops
          </div>
          <div className="mt-3 space-y-2">
            {dailyQuests.length ? dailyQuests.map(renderQuest) : (
              <div className="text-xs text-slate-500">No daily ops assigned.</div>
            )}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-800/80 bg-slate-950/50 p-4">
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">
            <Target className="h-4 w-4 text-violet-300" /> Weekly Campaign
          </div>
          <div className="mt-3 space-y-2">
            {weeklyQuests.length ? weeklyQuests.map(renderQuest) : (
              <div className="text-xs text-slate-500">No weekly ops assigned.</div>
            )}
          </div>
        </div>

        <div className="space-y-4">
          {/* Special Ops (event-driven) */}
          {specialOps.length > 0 && (
            <div className="rounded-2xl border border-violet-500/30 bg-violet-950/20 p-4">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-violet-300">
                <Zap className="h-4 w-4" /> Special Ops
              </div>
              <div className="mt-3 space-y-2">
                {specialOps.map(renderQuest)}
              </div>
            </div>
          )}
