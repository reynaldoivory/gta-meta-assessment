...
export const detectBottlenecks = (params, now, activeEvents, incomePerHour, formData) => {
  ...
  if (rank < 100 && autoShop2XEvent) {
    ...
    bottlenecks.push({
      ...
      detail: `Rank ${rank} = ~${maxHealthPercent}% max health. Before grinding Auto Shop finales: Max Strength (30 min), stock snacks/armor. Without prep: high failure rate on Union Depository/Lost MC finales. Prevents wasted time on failed missions.`,
  ...
  if (hasBunker && !bunkerUpgraded) {
    bottlenecks.push({
      id: 'bunker_upgrade',
      label: 'Bunker not upgraded',
      critical: false,
      impact: 'medium',
      detail: 'Unupgraded bunker production is ~64% slower.',
      timeHours: 0.5,
    });
  }

  if (hasNightclub && (nightclubTechs < 5 || nightclubFeeders < 5)) {
    bottlenecks.push({
      id: 'nightclub_partial',
      label: 'Nightclub not fully optimized',
      critical: false,
      impact: 'medium',
      detail: 'You are missing up to 50% of potential Nightclub passive income.',
      timeHours: 1,
    });
  }
