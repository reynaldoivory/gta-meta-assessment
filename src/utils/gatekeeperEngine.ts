...
export const checkGatekeeper = (taskId: string, userProfile: UserProfile) => {
    ...
    const missingAssets = rules.hard_gates.filter((asset: string) => !userProfile.assets.includes(asset));
    ...
    rules.soft_gates.forEach((gate: { stat: string; min: number; penalty: string }) => {
