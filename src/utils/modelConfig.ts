...
interface AssetsConfig {
  kosatka: number;
  kosatkaEfficiency: KosatkaEfficiency;
  agency: number;
  agencyDreContract: number;
  acidLab: number;
  nightclub: NightclubConfig;
}
...
interface ThresholdsConfig {
  ...
  nightclub: {
    ...
    maxFeeders: number;
    partialTechs: number;
    partialFeeders: number;
  };
  recommendations: {
    agencyPurchase: number;
  };
}
...
interface PriorityActionsConfig {
  strengthFix: number;
  flyingFix: number;
  kosatkaGrind: string;
  sparrowPurchase: string;
  agencySetup: string;
}

interface CayoConfig {
  basePayout: number;
  tequilaPayout: number;
  pinkDiamondPayout: number;
  masteryThreshold: number;
  masteryBonus: number;
  bagFillFactor: number;
  solo: {
    basePayout: number;
...
interface PassiveIncomeConfig {
  acidLabBase: number;
  acidLabUpgrade: number;
  acidLabUpgradeCost: number;
  nightclubMax: number;
  bunkerBase: number;
  bunkerUpgrade: number;
  bunkerUpgradeCost: number;
  salvageYard: number;
}

interface GtaPlusConfig {
  monthlyBonus: number;
  avgMonthlyHours: number;
  effectiveHourly: number;
}

interface AcidLabIncomeConfig {
  upgraded: {
    perHour: number;
  };
  unupgraded: {
...
interface SalvageYardConfig {
  robbery: number;
  towTruck: number;
}

interface NightclubIncomeConfig {
  max: number;
  partial: number;
  minimal: number;
}

interface BunkerIncomeConfig {
  upgraded: {
    perHour: number;
  };
  unupgraded: {
...
interface CarWashConfig {
  base: number;
  withWeedFarm: number;
  withHeliTours: number;
  maxSafe: number;
}

interface StreetDealersConfig {
  cocainePerDealer: number;
  methPerDealer: number;
  weedPerDealer: number;
  acidPerDealer: number;
  allTypesPerDealer: number;
  dealersPerDay: number;
  dailyTotal: number;
  dailyWithPremiums: number;
  timeMinutes: number;
}

interface CluckinBellConfig {
  payout: number;
  timeMinutes: number;
  perHour: number;
}
...
interface IncomeConfig {
  cayo: CayoConfig;
  agency: AgencyConfig;
  passive: PassiveIncomeConfig;
  gtaPlus: GtaPlusConfig;
  acidLab: AcidLabIncomeConfig;
  salvageYard: SalvageYardConfig;
  nightclub: NightclubIncomeConfig;
  bunker: BunkerIncomeConfig;
  autoShop: AutoShopConfig;
  carWash: CarWashConfig;
  streetDealers: StreetDealersConfig;
  cluckinBell: CluckinBellConfig;
}

interface DiversifiedIncomeConfig {
  points: {
    acidLab: number;
    agency: number;
    nightclub: number;
...
interface HeistReadinessConfig {
  totalCriteria: number;
  weights: {
    rank: number;
    strength: number;
    flying: number;
    diversifiedIncome: number;
    travelOptimized: number;
    bizCore: number;
  };
  diversifiedIncome: DiversifiedIncomeConfig;
}
...
export interface ModelConfig {
  ...
  time: TimeConfig;
  actionPlan: ActionPlanConfig;
  priorityActions: PriorityActionsConfig;
  income: IncomeConfig;
  heistReadiness: HeistReadinessConfig;
}
